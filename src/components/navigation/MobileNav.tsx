  ...
  return (
    <>
      <button 
        ...
        className="fixed top-6 right-6 w-12 h-12 flex items-center justify-center bg-brand-gold text-brand-green rounded-full shadow-2xl active:scale-90 transition-all z-[110] border border-brand-gold/20"
      >
        {isOpen ? <X size={20} /> : <Menu size={20} />}
      </button>

      <div className={cn(
        "fixed inset-0 bg-background z-[105] transition-all duration-700 ease-[cubic-bezier(0.19,1,0.22,1)] flex flex-col p-10 pt-24",
        ...
        <div className="relative z-10 space-y-1 mb-8">
          <p className="font-body text-[9px] tracking-[4px] text-brand-gold uppercase font-bold opacity-30">Strategic Deck</p>
            ...
            {sections.map((section, i) => (
              ...
              >
                ...
                >
                  ...
                  <ChevronRight className="text-brand-gold opacity-30" size={16} />
                </button>
              </li>
            ))}
          </ul>
        ...
        <div className={cn(
          "relative z-10 pt-6 border-t border-white/10 mt-auto transition-all duration-1000 delay-500",
          isOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
        )}>
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center">
              <Sparkles className="text-brand-gold opacity-20" size={16} />
            </div>
            <div>
              <p className="font-body text-[8px] text-white/40 font-bold uppercase tracking-widest leading-none">Firefly Management</p>
              <p className="font-body text-[7px] text-brand-gold/30 uppercase tracking-[2px] mt-1">Strategic Proposal</p>
            </div>
          </div>
        </div>
      </div>
