...
const RollingCounter = ({ value, suffix = "" }: { value: string, suffix?: string }) => {
  ...
  return (
    <span ref={containerRef} className="relative inline-block">
      <span className="font-headline text-7xl md:text-8xl text-brand-gold leading-none tracking-tighter drop-shadow-[0_0_20px_rgba(244,197,66,0.4)]">
        {Math.floor(displayValue)}{suffix}
      </span>
      <span className="absolute -inset-4 bg-brand-gold/5 blur-3xl rounded-full -z-10 animate-pulse" />
    </span>
  );
};
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 pt-6">
      {deliverables.map((item, i) => (
        <div 
          key={i} 
          className="relative group p-8 rounded-[32px] glass-tile overflow-hidden flex flex-col items-center text-center animate-float"
        ...
        >
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex flex-col items-center">
              ...
              <span className="font-headline text-xl text-brand-gold/60 uppercase tracking-widest -mt-2">
                {item.period}
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="font-body text-brand-cream/70 text-xs leading-relaxed font-bold uppercase tracking-wider max-w-[240px] mx-auto">
                {item.body}
              </p>
            </div>
          </div>
          <div className="absolute inset-x-0 bottom-0 h-1 bg-brand-gold/20 scale-x-0 group-hover:scale-x-100 transition-transform duration-700" />
        </div>
      ))}
    </div>
  );
