...
export const ContentSocialSection = () => {
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="space-y-4 animate-fade-in-up">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "Sonic Eng", icon: <Mic2 />, desc: "Mixer-feed capture." },
          ].map((role, i) => (
            <Card key={i} className="p-4 bg-white border border-brand-green/5 rounded-xl space-y-2 hover:border-brand-gold transition-all shadow-sm">
              <div className="w-8 h-8 rounded-full bg-brand-gold/10 text-brand-gold flex items-center justify-center">{role.icon}</div>
      ...
      {/* 04B: Sonic Pipeline Visual - Compact */}
      <div className="space-y-4 animate-fade-in-up">
        <div className="section-label">04B: The Sonic Pipeline</div>
        <div className="bg-brand-cream/50 border border-brand-green/5 rounded-[24px] p-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 relative">
            ...
            {[
              ...
              <div key={i} className="relative z-10 flex flex-col items-center text-center space-y-2">
                <div className="w-10 h-10 rounded-full bg-white border border-brand-green/5 shadow-sm flex items-center justify-center text-brand-teal">
                  {step.icon}
                </div>
            ...
            <TableBody>
              {[
                ...
                <TableRow key={i} className="hover:bg-brand-gold/5 transition-colors border-brand-green/5">
                  <TableCell className="font-headline text-lg py-3 px-4 text-brand-teal">{row.time}</TableCell>
