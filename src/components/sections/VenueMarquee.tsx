...
export const Tier1Marquee = () => {
  ...
  const transformation = 'q_auto:best,f_auto,dpr_2.0,h_480,c_limit';
          ...
          {displayVenues.map((venue, i) => {
            ...
            return (
              <React.Fragment key={`${venue.name}-${i}`}>
                <div className="flex flex-col items-center gap-4 shrink-0 transition-transform duration-500 hover:scale-110 will-change-transform">
                  {logo && (
                    <img 
                      src={cl(logo.imageUrl, transformation)} 
                      alt={venue.name}
                      className="h-[120px] md:h-[240px] w-auto block flex-shrink-0 object-contain drop-shadow-[0_30px_70px_rgba(0,0,0,0.9)] brightness-125"
                      loading="lazy"
                      decoding="async"
                    />
                  )}
...
export const Tier2Marquee = () => {
  ...
  const transformation = 'q_auto:best,f_auto,dpr_2.0,h_220,c_limit';
